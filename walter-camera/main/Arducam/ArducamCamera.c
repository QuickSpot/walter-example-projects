...
#define arducamSpiTransfer(cam, val) spiReadWriteByte(cam, val)
...
#define arducamDelayMs(ms) vTaskDelay(pdMS_TO_TICKS(ms))
#define arducamDelayUs(us) ets_delay_us(us)
...
void cameraWaitI2cIdle(ArducamCamera* camera) {
  uint32_t timeout = 10000; // Timeout in milliseconds (adjust as necessary)
  while ((readReg(camera, CAM_REG_SENSOR_STATE) & 0X03) != CAM_REG_SENSOR_STATE_IDLE) {
    if (--timeout == 0) {
      ESP_LOGE("camera", "Timeout waiting for I2C idle state.");
      break; // Exit after timeout
    }
    arducamDelayMs(2); // Wait for 2 ms before checking again
  }
}
