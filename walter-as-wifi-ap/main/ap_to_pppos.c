 ...
 * @brief Wi-Fi soft AP to LTE cellular bridge using PPPoS on Walter
 *
 * @section LICENSE
 *
 ...
 * @section DESCRIPTION
 *
 * This example turns Walter into a cellular Wi-Fi hotspot. The Sequans GM02SP
 * modem is driven over UART with hardware flow control using the esp-modem
 * component in PPPoS mode. Once a PPP link is established, a Wi-Fi soft AP is
 * started and IP NAT is enabled so that connected stations can reach the
 * internet through the LTE uplink. DNS information received from the LTE
 * network is forwarded to Wi-Fi clients via the DHCP server. The firmware
 * monitors the PPP link and automatically reconnects whenever the connection
 * is lost, including signal-quality checks and modem reset as fallback.
 */
